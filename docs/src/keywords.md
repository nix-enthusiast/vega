# Keywords

Keywords are reserved words that provide core functionality to the programming language.

Vega has 23 keywords, each linked to its relevant section.

| Keyword    | Look At                                                                                                          |
|------------|------------------------------------------------------------------------------------------------------------------|
| `run`      | [Multithreading and Concurrency](./multithreading_and_concurrency.md)                                            |
| `return`   | [Functions and Behaviors](./functions_and_behaviors.md)                                                          |
| `cond`     | [Conditional Branching](./control_flow.md)                                                                       |
| `try`      | [Error Handling](./error_handling.md)                                                                            |
| `catch`    | [Error Handling](./error_handling.md)                                                                            |
| `end`      | [Scopes](./variables_and_scopes/scopes.md)                                                                       |
| `is`       | [Reflection](./polymorphism/reflection.md)                                                                       |
| `and`      | [Conditional Branching](./control_flow.md) and [math](./standard_library/math.md)                                |
| `or`       | [Conditional Branching](./control_flow.md) and [math](./standard_library/math.md)                                |
| `xor`      | [Conditional Branching](./control_flow.md) and [math](./standard_library/math.md)                                |
| `not`      | [Conditional Branching](./control_flow.md) and [math](./standard_library/math.md)                                |
| `true`     | [Control Flow](./control_flow.md)                                                                                |
| `false`    | [Control Flow](./control_flow.md)                                                                                |
| `nil`      | [Variables](./variables_and_scopes_and_namespace/variables.md)                                                   |
| `noreturn` | [Functions](./functions_and_behaviors/functions.md)                                                              |
| `mut`      | [Variables](./variables_and_scopes/variables.md)                                                                 |
| `maybe`    | [Variables](./variables_and_scopes/variables.md)                                                                 |
| `!`        | [Error Handling](./error_handling.md)                                                                            |
| `pub`      | [Orb](./the_orbit_package_manager/orb.md)                                                                        |
| `import`   | [Importing Orbs](./the_orbit_package_manager/importing_orbs.md)                                                  |
| `as`       | [Importing Orbs](./the_orbit_package_manager/importing_orbs.md) and [behaviors](./standard_library/behaviors.md) |
| `external` | [FFI](./ffi.md)                                                                                                  |
